...
export default function ResultsPage({ params }: { params: Promise<{ wallet: string }> }) {
  const router = useRouter()
  const auth = useGithubAuth()
  const { wallet: rawWallet } = use(params)
  // Query string is read on the client so the server segment can stay
  // statically cacheable — `searchParams` on the page would force dynamic
  // rendering of every `/score/[wallet]?…` variant.
  const query = useSearchParams()
  const github = query.get('github') ?? undefined
  const wallets = query.get('wallets') ?? undefined
  let wallet: string
  try {
